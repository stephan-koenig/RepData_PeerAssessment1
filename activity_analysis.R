...
dateDownloaded <- "Fri Jan 22 16:58:12 2016"
...
activityTotalPerDay <- na.omit(activity) %>% group_by(date) %>%
        summarize(totalSteps = sum(steps))
activityTotalPerDay <- arrange(activityTotalPerDay, totalSteps)
# Make a histogram of the total number of steps taken each day


# Calculate mean and median of the total steps taken per day
meanSteps <- mean(activityTotalPerDay$totalSteps)
medianSteps <- median(activityTotalPerDay$totalSteps)
